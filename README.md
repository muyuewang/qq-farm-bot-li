...
- 支持萌宠成长日记（S3）全部玩法的自动化，通过设置页10个独立开关控制
- 自动领养比熊、投喂（最多12次直到成年）、领取成年比熊
- 自动寻宝（最多12次）、领取爪印手记、领取种子礼包、领取节令好礼
- 自动开启已完成的宝藏护送、领取夺宝补偿
- 自动选择锦囊（投喂后、寻宝前，取池中第一个）
- 自动好友夺宝巡查：遍历好友列表，按低→高优先级使用挑战书（80101 > 80102 > 80103），30秒预算内执行
- 宝藏护送完成时精准唤醒（不依赖5分钟轮询），避免白等
- 所有操作串行执行，失败只记日志不中断后续步骤
- 拒绝使用钻石，保护付费资源
...
| 开关 | 默认值 | 说明 |
| --- | --- | --- |
| `pet_diary_adopt` | 关 | 自动领养比熊并领取成年比熊 |
| `pet_diary_feed` | 关 | 自动投喂直到成年（7000成长值，约10次） |
| `pet_diary_draw` | 关 | 自动寻宝（每日上限10次） |
| `pet_diary_story_claim` | 关 | 自动领取已解锁的爪印手记 |
| `pet_diary_seed_claim` | 关 | 自动领取每日种子礼包 |
| `pet_diary_solar_claim` |关 | 自动领取当前可领的节令好礼 |
| `pet_diary_treasure_open` | 关 | 自动开启已完成护送的宝藏 |
| `pet_diary_compensation_claim` | 关 | 自动领取夺宝补偿 |
| `pet_diary_charm_equip` | 关 | 自动选择锦囊（取池中第一个） |
| `pet_diary_battle` | 关 | 自动好友夺宝（会消耗挑战书并掠夺好友宝藏） |

自动化每5分钟执行一次。开启任意开关后，系统会在登录15秒后首次执行，之后按5分钟间隔循环。宝藏护送完成时会精准唤醒，不依赖固定轮询。
...
- [萌宠成长日记协议与实现](docs/pet-diary-20260910.md)
