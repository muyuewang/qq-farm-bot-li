...
Code 具有时效性；登录失败时应先重新获取 Code 或重新扫码。会话已经建立、之后被外侧断开（`heartbeat_timeout`、`ws_close` 这类 `phase=online` 的掉线）时，主进程会先用原 Code 把该账号重启一次，试一次仍不通才发下线提醒交人工；握手阶段就被拒（`400`）和被其他终端踢下线不会自动重试，同一个 Code 十分钟内的重试上限是一次。QQ 客户端被挤下线后，官方“重新登录”会签发新的 Code；可启动 QQFarmCodeHelper 的标准获取模式并开启自动同步，再点击官方“重新登录”，Helper 会更新匹配账号的 Code 并重新启动远程账号。协议监听模式只落盘抓包，永远不会自动上传 Code。
