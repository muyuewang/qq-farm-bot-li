...
interface TrafficWindowStats {
    ...
    topMethods: Array<[string, number]>;
}

function emptyBucket(sec: number): TrafficBucket {
    return { sec, outbound: 0, responseIn: 0, pushIn: 0, byClass: new Map(), byMethod: new Map() };
}

function mergeCount(target: Map<string, number>, key: string, delta: number): void {
...
function createTrafficMeter() {
    ...
    for (let i = 0; i < bucketCount; i++) buckets.push(emptyBucket(-1));

    function bucketFor(now: number): TrafficBucket {
    ...
    function recordOutbound(requestClass: string | null | undefined, methodName: string | null | undefined, now: number = Date.now()): void {
    ...
    function recordInbound(kind: 'response' | 'push', now: number = Date.now()): void {
        const bucket = bucketFor(now);
        if (kind === 'push') bucket.pushIn += 1;
        else bucket.responseIn += 1;
    }

    /** 清零。新连接建立时调用，避免把上一条连接的速率算进来。 */
    function reset(now: number = Date.now()): void {
        const sec = Math.floor(now / TRAFFIC_BUCKET_MS);
    ...
    function snapshot(windowMs: number, now: number = Date.now()): TrafficWindowStats {
        ...
        for (let sec = startSec; sec <= endSec; sec++) {
            ...
            for (const [k, v] of bucket.byClass) mergeCount(byClass, k, v);
            for (const [k, v] of bucket.byMethod) mergeCount(byMethod, k, v);
        }
    ...
    function formatWindow(windowMs: number, now: number = Date.now()): string {
