...
function handleMessage(data) {
    try {
        ...
        if (msgType === 3)
            trafficMeter.recordInbound('push');
        else if (msgType === 2)
            trafficMeter.recordInbound('response');
        // Notify
        if (msgType === 3) {
            handleNotify(msg);
            return;
...
function startHeartbeat(context) {
    ...
    networkScheduler.setIntervalTask('traffic_stats', TRAFFIC_STATS_LOG_INTERVAL_MS, () => {
        ...
        if (!isCurrentConnection(context) || context.phase !== 'online')
            return;
        const stats = trafficMeter.snapshot(TRAFFIC_STATS_LOG_INTERVAL_MS);
        if (stats.outbound + stats.responseIn + stats.pushIn === 0)
            return;
    ...
    networkScheduler.setIntervalTask('heartbeat_interval', CONFIG.heartbeatInterval, async () => {
        ...
        try {
            ...
            logWarn('心跳', `心跳未响应 (miss=${heartbeatMissCount}/${MAX_HEARTBEAT_MISSES}, `
                ...
                + `pending=${pendingCallbacks.size}, queued=${requestQueue.length}, active=${describePendingRequests()}, `
                + `近60s ${trafficMeter.formatWindow(60000)})`, { gid: userState.gid });
