...
function parseCpuStat(text) {
    ...
    for (const line of String(text || '').split('\n')) {
        ...
        if (key === 'nr_throttled')
            out.nrThrottled = Number(value) || 0;
        else if (key === 'throttled_usec')
            out.throttledUs = Number(value) || 0;
    }
    return out;
}
...
function createStallProbe(options = {}) {
    ...
    function classify(report, stallMs) {
        if (report.skewMs > clockSkewWarnMs) {
            return `时钟跳变或主机挂起（墙钟比单调钟多走 ${round1(report.skewMs / 1000)}s，单调钟不计挂起时间）`;
        }
        if (!report.stalled)
            return `定时器没迟到，但事件循环出现过同步阻塞（峰值 ${round1(report.loopDelayMaxMs / 1000)}s）`;
        if (report.activeMs >= stallMs * 0.6)
            return `事件循环被同步代码占住（活跃 ${round1(report.activeMs / 1000)}s）`;
        if (report.throttledUs > 0)
            return `容器 CPU 配额被打满（限流 ${round1(report.throttledUs / 1000)}ms/${report.nrThrottled} 次）`;
...
//# sourceMappingURL=stall-probe.js.map