...
const MERGED_DATA_METADATA = [67371008, 404];
...
function resolveTsdkPlatform(value) {
    const normalized = String(value || '').trim().toLowerCase();
    return normalized === 'wx' || normalized === 'wechat' ? 'wx' : 'qq';
}
function resolveTsdkHostProfile(value) {
...
class TsdkRuntime {
    ...
    getDeviceText() {
        if (this.hostProfile.deviceText)
            return this.hostProfile.deviceText;
    ...
    createImports() {
        return {
            a: {
                ...
                i: (ptr, capacity) => this.writeCString(this.hostProfile.userDataPath || `${this.dataDir}${path.sep}`, ptr, capacity),
                ...
                m: (ptr, capacity) => this.writeCString(this.hostProfile.appId, ptr, capacity),
                n: (ptr, capacity) => this.writeCString(this.hostProfile.appId, ptr, capacity),
    ...
    async init() {
        ...
        this.initPromise = (async () => {
            ...
            if (typeof decryptAllData !== 'function')
                throw new Error('TSDK 缺少 mergewasm 全量数据解密导出');
    ...
    normalizeQqHostFeatureState() {
        if (this.hostProfile.platform !== 'qq')
            return;
        const { currentPtr, referencePtr, length, nodeMismatchIndex, } = QQ_HOST_FEATURE_STATE;
        this.ensureBounds(currentPtr, length);
        this.ensureBounds(referencePtr, length);
        const view = this.view();
        const mismatches = [];
        for (let index = 0; index < length; index += 1) {
            if (view[currentPtr + index] !== view[referencePtr + index])
