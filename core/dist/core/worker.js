...
function stopAutumnActivityTimer() {
...
async function runPetDiaryTick() {
    if (!loginReady)
        return;
    const automation = getAutomation() || {};
    if (!isPetDiaryAutomationEnabled(automation))
        return;
...
function startPetDiaryTimer() {
    stopPetDiaryTimer();
    if (!loginReady || !isPetDiaryAutomationEnabled(getAutomation()))
        return;
...
let autumnAutomation = null;
async function runAutumnActivityTick() {
    if (!loginReady)
        return;
    const automation = getAutomation() || {};
    if (!isAutumnAutomationEnabled(automation))
        return;
    if (!autumnAutomation) {
...
function startAutumnActivityTimer() {
    stopAutumnActivityTimer();
    if (!loginReady || !isAutumnAutomationEnabled(getAutomation()))
        return;
