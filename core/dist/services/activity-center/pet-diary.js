...
const pet_diary_2026090101_json_1 = __importDefault(require("../../activity-data/pet-diary-2026090101.json"));
const pet_diary_assets_json_1 = __importDefault(require("../../activity-data/pet-diary-assets.json"));
const GROUP_ID = '2026090100';
const PET_ID = '2026090101';
const SEEDS_ID = '2026090102';
const SHOP_ID = '2026090103';
const DIAMOND_ID = '1004';
const base = pet_diary_2026090101_json_1.default.ActivityPetTreasureHuntBase[0];
const fight = pet_diary_2026090101_json_1.default.ActivityPetTreasureHuntFight[0];
const refresh = pet_diary_2026090101_json_1.default.ActivityPetTreasureCharmRefresh[0];
const assetPaths = new Map(pet_diary_assets_json_1.default.map(entry => [entry.path.replace(/\/spriteFrame$/, ''), `/activity-assets/pet-diary/${entry.file}`]));
...
function createPetDiaryService(deps) {
    const { types, sendMsgAsync, getBag, getBagItems, getServerTimeSec, itemDto, int64String: str, int64Number: num, textContent, businessError, positiveDecimal, serializeMutation, getCurrentSolarTerms } = deps;
    ...
    const plain = (type, value) => types.PetDiaryOperateReply.root.lookupType(`gamepb.activitypb.${type}`).toObject(value, { longs: String, defaults: true });
    ...
    function costsAvailable(costs, bag, count = '1') {
        ...
        for (const item of costs) {
            const id = str(item.id);
            if (id === DIAMOND_ID || id === '0' || BigInt(str(item.count)) <= 0n)
                return false;
    ...
    function normalize(group, bag, solar = null, warnings = []) {
        ...
        const charmDto = (id) => {
            const config = pet_diary_2026090101_json_1.default.ActivityPetTreasureHuntCharm.find(entry => entry.charm_id === id);
        ...
        return {
            ...
            stories: list(state.story?.stories).map(s => {
                ...
                return { order: num(s.order), unlocked: s.unlocked === true,
                    claimed: s.claimed === true, animated: s.animated === true, photo: localImage(desc.photo) };
            }),
            charms: { pool: list(battle.charm_daily_pool).map(charmDto), equipped: list(battle.charm_equipped).map(charmDto), all: pet_diary_2026090101_json_1.default.ActivityPetTreasureHuntCharm.map(c => charmDto(c.charm_id)),
            ...
            shop: goods, solarTerms: solar ? { ...solar, terms: list(solar.terms).filter(term => (Number(term.endTime) >= num(group.pet.head.start_time) && Number(term.startTime) <= num(group.pet.head.end_time))) } : null,
    ...
    async function readSnapshot() {
        ...
        try {
            ...
            warnings.push('背包读取失败，消耗资源的操作已暂停');
        }
        try {
            solar = await getCurrentSolarTerms();
        }
        catch {
            warnings.push('节令小礼读取失败，请稍后刷新');
        }
    ...
    async function operatePetDiary(actionInput, input = {}) {
        if (actionInput === 'solar')
            return claimPetDiarySolarTerm(input?.termId);
        if (typeof actionInput !== 'string' || !Object.hasOwn(OPERATIONS, actionInput))
            fail('未知萌宠操作');
        const action = actionInput;
        return serializeMutation(async () => {
            const group = await readGroup();
            if (!isActive(group.pet.head))
                fail('萌宠成长日记当前不在活动时间内');
            const state = group.pet.pet_treasure_hunt;
            const nurture = state.nurture || {};
            const battle = state.battle || {};
            let params = {};
            ...
            if (action === 'feed' || action === 'draw') {
                ...
                if (!costsAvailable(costs, await balances()))
                    fail('萌宠元气糕不足，请先种植活动作物');
            }
                ...
                if (!isActive(group.seeds?.head) || !list(group.seeds?.mega_event?.rewards).some(r => r.claimable && !r.claimed))
                    fail('当前没有可领取的种子礼包');
                id = SEEDS_ID;
            }
            else if (action === 'refreshCharm') {
                if (num(nurture.stage) !== 2)
                    fail('比熊成年后才可刷新锦囊');
                if (charmNeedsChoice(battle))
                    fail('请先替换或保留当前锦囊，再刷新');
                ...
                if (input?.allowDiamonds || (input?.payment && !['free', 'tickets'].includes(input.payment)))
                    fail('锦囊刷新不支持使用钻石');
                if (free) {
                    if (input?.payment && input.payment !== 'free')
                        fail('刷新次数已变化，请刷新状态后重试');
                }
                else {
                    if (input?.payment !== 'tickets')
                        fail('免费刷新已用完，请确认点券费用后再操作');
                    const paidCount = num(battle.charm_paid_refresh_count);
                    if (paidCount >= refresh.manual_refresh_daily_limit)
                        fail('今日付费刷新次数已用完');
                ...
                if (!list(state.pool?.treasures).some(t => num(t.status) === 3 || (num(t.status) === 2 && num(t.end_at) > 0 && num(t.end_at) <= getServerTimeSec())))
                    fail('还没有完成护送的宝藏');
            }
                ...
                if (BigInt(str(goods.purchase_limit)) > 0n && BigInt(str(goods.purchased_count)) + BigInt(count) > BigInt(str(goods.purchase_limit)))
                    fail('兑换数量超过剩余限购次数');
                if (!costsAvailable(list(goods.cost), await balances(), count))
                    fail('兑换余额不足');
                ...
                if (![80101, 80102, 80103].includes(Number(challengeId)))
                    fail('挑战书类型无效');
                const friend = await getPetDiaryFriend(gid);
                const treasure = friend.treasures.find(t => t.id === String(input?.treasureId));
                if (!treasure || treasure.status !== 2 || !treasure.previews.some(p => p.challengeId === challengeId && p.canStart))
                    fail('好友宝藏状态已变化，请重新查看');
                if (!costsAvailable([{ id: challengeId, count: '1' }], await balances()))
                    fail('对应挑战书不足');
                ...
                if (!orders.length || orders.some(order => !list(state.story?.stories).some(s => num(s.order) === order && s.unlocked)))
                    fail('手记编号无效');
                params = { orders };
            }
    ...
    async function claimPetDiarySolarTerm(termIdInput) {
        ...
        return serializeMutation(async () => {
            ...
            if (str(reply.term?.term_id) !== termId || num(reply.term?.status) !== 3)
                fail('节令回包不匹配，请刷新确认领取状态');
            let snapshot = null;
            let refreshError = '';
            try {
                snapshot = await readSnapshot();
...
//# sourceMappingURL=pet-diary.js.map