...
async function webUiLogin() {
    if (webUiCredential)
        return webUiCredential;
    const token = authToken();
    if (!token)
        throw new Error('NapCat Token 文件不存在或为空');
...
async function requestWebUI(path, body = {}) {
...
async function getMiniappCode(taskId) {
    ...
    if (task.result)
        return { code: task.result.code, nickname: task.result.nickname };
    ...
    // 获取小程序授权码
    const result = await axios.post(apiUrl(openAuthEndpoint, '/miniapp'), { appId: QQ_MINIAPP_APP_ID, interactive: true }, {
        timeout: REQUEST_TIMEOUT_MS,
        headers: {
            'Content-Type': 'application/json',
