...
function getAvailableItemCount(bagReply, itemId, uid) {
...
function getDogDefinition(id, raw, currentDogId, skillUsages, bagReply) {
...
function buildPetSnapshot(reply, bagReply) {
    ...
    const dogs = PET_IDS.map((id) => getDogDefinition(id, byId.get(id) || {}, currentDogId, skillUsages, bagReply));
    // 保留服务端新增的宠物，避免客户端配置尚未更新时静默丢失数据。
    for (const raw of rawDogs) {
        const id = normalizeId(raw?.id);
        if (id > 0 && !PET_IDS.includes(id))
            dogs.push(getDogDefinition(id, raw, currentDogId, skillUsages, bagReply));
    }
...
async function activateDog(dogIdInput) {
    ...
    const pet = before.dogs.find((entry) => entry.id === dogId);
    if (!pet)
        throw new Error('该宠物不在图鉴中');
    if (pet.owned)
        throw new Error('该宠物已获得，无需重复激活');
    if (!pet.activatable)
        throw new Error('背包中没有该宠物的卡片，无法激活');
    // 真实抓包：DogService.ActivateDog({ field 1: dog_id }) 会消耗背包中的宠物卡，
    // 并把图鉴项标记为已获得（GetDogInfo field 7），之后才能真正上场。
    const body = types.ActivateDogRequest.encode(types.ActivateDogRequest.create({ dog_id: dogId })).finish();
    ...
    const activated = snapshot.dogs.find((entry) => entry.id === dogId);
    if (!activated?.owned)
        throw new Error('宠物激活状态未更新，请稍后重试');
...
async function deployDog(dogIdInput) {
    ...
    if (!dog)
        throw new Error('该宠物不在图鉴中');
    if (!owned)
        throw new Error(normalizeId(dog?.field_6) === 1 ? '该宠物尚未激活，请先激活' : '未获得该宠物，无法上场');
