...
function isAutumnAutomationEnabled(automation = {}) {
...
function createAutumnAutomation(deps) {
    ...
    function skip(action, text) {
        const key = `${todayKey()}:${action}`;
        if (skipLoggedOn.get(action) === key)
            return;
    ...
    function rewardText(result) {
        ...
        return rewards.length ? `，获得 ${rewards.map((item) => `${item.name}x${item.count}`).join('、')}` : '';
    }
    async function perform(key, action, title, input) {
        try {
    ...
    async function runWish(flags) {
        ...
        const picked = (state.choices || []).find((item) => item.id === choice);
        if (!picked) {
    ...
    async function runHappy(flags) {
        ...
        if (flags.daily) {
            if (state.canClaimDaily)
                await perform('happy', 'daily', '快乐不独享领取每日快乐值');
            else
                skip('happy_daily', '今日快乐值已领取');
        }
        if (flags.share) {
            if (state.canShare)
                await perform('happy', 'share', '快乐不独享领取首次分享奖励');
            else
                skip('happy_share', '今日分享奖励已领取');
        }
        if (flags.milestones) {
            if (state.canClaimMilestones)
                await perform('happy', 'milestones', '快乐不独享领取档位奖励');
            else
                skip('happy_milestones', '没有可达标的档位');
        }
    }
    ...
    async function runAutumnAutomation(flags) {
        ...
        if (flags.wishDraw || flags.wishClaim) {
            tasks.push(runWish({ draw: flags.wishDraw, claim: flags.wishClaim, choice: flags.wishChoice })
                .catch((e) => { log('活动', `秋祈良愿自动任务读取状态失败: ${e.message}`, { module: 'activity', event: '秋日活动自动', result: 'error', key: 'wish' }); }));
        }
        if (flags.happyDaily || flags.happyShare || flags.happyMilestones) {
            tasks.push(runHappy({ daily: flags.happyDaily, share: flags.happyShare, milestones: flags.happyMilestones })
                .catch((e) => { log('活动', `快乐不独享自动任务读取状态失败: ${e.message}`, { module: 'activity', event: '秋日活动自动', result: 'error', key: 'happy' }); }));
        }
...
//# sourceMappingURL=autumn-activities-auto.js.map