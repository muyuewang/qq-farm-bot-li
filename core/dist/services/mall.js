...
async function purchaseMallGoods(goodsId, count = 1) {
    ...
    if (reply.success !== true)
        throw new Error('商城未确认购买成功，请刷新核对');
    return reply;
}
...
async function buyFreeGifts(force = false) {
    ...
    try {
        ...
        const free = goods.filter((g) => !!g && g.is_free === true && g.is_available === true && !g.ad_only && !g.share?.share_only && toNum(g.price?.count) === 0 && Number(g.goods_id || 0) > 0);
        if (!free.length) {
