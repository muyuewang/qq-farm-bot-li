...
export declare function getFriendsList(forceSync?: boolean, priority?: 'low' | 'normal', propagateErrors?: boolean): Promise<any[]>;
export declare function getFriendsListCacheOnly(): any[];
/**
 * 获取指定好友的农田详情 (进入-获取-离开)
 */
export declare function getFriendLandsDetail(friendGid: number, propagateErrors?: boolean): Promise<any>;
export declare function runBatchWithFallback(ids: number[], batchFn: (ids: number[]) => Promise<any>, singleFn: (ids: number[]) => Promise<any>, propagateErrors?: boolean): Promise<number>;
/**
 * 面板手动好友操作（单个好友）
 * opType: 'steal' | 'water' | 'weed' | 'bug' | 'bad'
 */
export declare function doFriendOperation(friendGid: any, opType: string, propagateErrors?: boolean): Promise<any>;
