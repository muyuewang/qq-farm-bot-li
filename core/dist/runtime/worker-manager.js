...
function createWorkerManager(options) {
     ...
     */
    function shouldRetryWithStoredCode(accountId, account, phase, handshakeCode) {
        if (phase !== 'online')
            return false;
        if (handshakeCode === 400)
            return false;
        const code = String(account && account.code ? account.code : '');
        if (!code)
            return false;
        const rec = codeRetryStates.get(accountId);
        if (rec && rec.code === code && Date.now() - rec.at < codeRetryMinIntervalMs)
            return false;
        return true;
    }
    ...
    function handleWorkerMessage(accountId, sourceProcess, msg) {
        ...
        if (msg.type === 'status_sync') {
            ...
            if (!worker.stopping && shouldRetryWithStoredCode(accountId, worker.accountRef, phase, code)) {
                ...
                addAccountLog('disconnect_retry', `账号 ${worker.name} 连接已断开，先用原 Code 重启试一次`, accountId, worker.name, { source, code, reason, phase });
                stopWorker(accountId);
